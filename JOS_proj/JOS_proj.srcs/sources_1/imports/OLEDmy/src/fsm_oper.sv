...
localparam nb_screens = 1'b10, // ilość scrennów
	nb_pages = 3'b100,  //
	nb_letters = 5'b10000,
	nb_columns = 4'b1000;
...
//wczytuj dane z data_out do uart_screen, resztę wypełnij zerami
always @(posedge clk, posedge rst)
	if(rst)
		uart_screen[cnt_page][cnt_ind] <= 1'b0;
	else if(st == screen)
		uart_screen[0][0] <= data_out;
	...
	if(rst)
	   ...
	   //current_screen <= alphabet_screen;
		case(cnt_screen)
			...
			2'b01: current_screen <= uart_screen; 
